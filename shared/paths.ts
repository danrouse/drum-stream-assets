...
export const DOWNLOADS_PATH = process.env.DOWNLOADS_PATH
  ? resolve(process.env.DOWNLOADS_PATH)
  : join(__dirname, '..', 'music-stem-server', 'library', 'downloads');
export const DEMUCS_OUTPUT_PATH = process.env.DEMUCS_OUTPUT_PATH
  ? resolve(process.env.DEMUCS_OUTPUT_PATH)
  : join(__dirname, '..', 'music-stem-server', 'library', 'separated');
