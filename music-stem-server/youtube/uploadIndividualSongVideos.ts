...
discordModule.on('ready', async (client) => {
  ...
  for (let filename of readdirSync(RECORDINGS_PATH)) {
    ...
    while (!lastMessageTime || (streamTime < lastMessageTime)) {
      ...
      const previousSize = discordModule.songRequestsChannel?.messages.cache.size;
...
Song request 🆔 ${songRequestId}`;
    ...
    await discordModule.updateCompletedSongRequest(Number(songRequestId), undefined, youtubeUrl);
